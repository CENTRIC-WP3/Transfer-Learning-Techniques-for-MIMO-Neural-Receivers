## this file trains in c 64QAM and evaluate in same
...
#####
def without_tl(chann_model, subc_space, alf,bit_per_sys):
############################################
    ...
    num_bits_per_symbol = bit_per_sys#6#4#2 # 64QAM
    ...
    model_weights_path = "without_TL_wghts" # Location to save the neural receiver weights once training is done

    ############################################
    ## Evaluation configuration
    #results_filename = "nnrx_results_layer_plusWTL" # Location to save the results
    ...
    class E2ESystem2(Model):
        ...
        and the parameter ``training`` is used to specified if the system is instantiated to be trained or to be evaluate_0.1Ns_0.1Ns_0.1Nsd.
    ...
    ## Receiver
    neural_receiver = NeuralReceiver2()
    ...
    model_tf_layer_plus = E2ESystem2('neural-receiver',training = True)
    ...
    for i in range(num_training_iterations):
        ...
        with tf.GradientTape() as tape:
            rate = model_tf_layer_plus(training_batch_size, ebno_db)
        ...
        weights = model_tf_layer_plus.trainable_weights
    ...
    weights = model_tf_layer_plus.get_weights()
    with open(model_weights_path, 'wb') as f:
        pickle.dump(weights, f)

    #############################################################    
    ###### evaluate the model for the WITHOUT TL Case############
    ...
    model_layer_plus = E2ESystem2('neural-receiver')

    # Run one inference to build the layers and loading the weights from the target model
    model_layer_plus(1, tf.constant(10.0, tf.float32))
    with open(model_weights_path, 'rb') as f:
        weights = pickle.load(f)
    model_layer_plus.set_weights(weights)
    ...
    _,bler = sim_ber(model_layer_plus, ebno_dbs, batch_size=128, num_target_block_errors=100, max_mc_iter=10000)
